 ...
 * Un compromiso ya vencido (hoursRemaining <= 0) deja de ser "urgente": la
 * ventana de urgencia es la aproximación al vencimiento, no el vencimiento en
 * sí (ver CLAUDE.md — decisión 2026-09-04).
 */
export function isUrgentByDeadline(dueAt: Date | null | undefined, now: Date = new Date()): boolean {
  ...
  return hoursRemaining > 0 && hoursRemaining <= URGENT_THRESHOLD_HOURS;
}
...
// Normaliza el tiempo restante a un score 0..1: sin compromiso -> 0, ya
// vencido -> 0 de inmediato (un compromiso vencido sin resolver deja de
// competir por los primeros lugares de la bandeja priorizada — sigue
// accionable en Prioridades y en el conteo de "vencidos" del Informe, ver
// CLAUDE.md), inminente -> cerca de 1, decayendo conforme el plazo se aleja.
